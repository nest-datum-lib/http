...
export class UserService extends MainService {
	...
	async register(payload): Promise<any> {
		...
		try {
			...
			this.repositoryCache.drop({ key: [ this.prefix(process.env.SERVICE_SSO), 'many', '*' ] });
	...
	async verify(payload): Promise<any> {
		...
		this.repositoryCache.drop({ key: [ this.prefix(process.env.SERVICE_SSO), 'many', '*' ] });
		this.repositoryCache.drop({ key: [ this.prefix(process.env.SERVICE_SSO), 'one', { id: user['id'] } ] });
